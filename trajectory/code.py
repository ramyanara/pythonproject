    ...
    for currentFrameBlob in currentFrameBlobs:
        ...
        if (dblLeastDistance < (currentFrameBlob.dblCurrentDiagonalSize * 1.0) / 1.2):
            ...
            if len(blobs[i].centerPositions) >= 2:
                ...
                if (blobs[i].centerPositions[prevFrameIndex][-1] > Line2[0][1] and
                        blobs[i].centerPositions[currFrameIndex][-1] <= Line2[0][1] and
                        blobs[i].centerPositions[prevFrameIndex][-2] >= Line2[0][0] and
                        blobs[i].centerPositions[currFrameIndex][-2] >= Line2[0][0]):
                    # print "Entered"
                    intime[i] = datetime.datetime.now()
                if (blobs[i].centerPositions[prevFrameIndex][-1] > Line1[0][1] and
                        blobs[i].centerPositions[currFrameIndex][-1] <= Line1[0][1] and
                        blobs[i].centerPositions[prevFrameIndex][-2] >= Line2[0][0] and
                        blobs[i].centerPositions[currFrameIndex][-2] >= Line2[0][0]):
                    # print "Exit"
...
def checkIfBlobsCrossedTheLine(blobs, Line2, carCount, carCount4):
    ...
    for blob in blobs:
        if (blob.blnStillBeingTracked == True and len(blob.centerPositions) >= 2):
            ...
            if (blob.centerPositions[prevFrameIndex][-1] > Line2[0][1] and blob.centerPositions[currFrameIndex][-1] <=
                    Line2[0][1] and blob.centerPositions[prevFrameIndex][-2] >= Line2[0][0] and
                    blob.centerPositions[currFrameIndex][-2] >= Line2[0][0]):
                carCount += 1
...
def checkIfBlobs2CrossedTheLine(blobs, Line1, carCount2, carCount3):
    ...
    for blob in blobs:
        if (blob.blnStillBeingTracked == True and len(blob.centerPositions) >= 3):
            ...
            currFrameIndex = len(blob.centerPositions) - 1
            if (blob.centerPositions[prevFrameIndex][-1] > Line1[0][1] and blob.centerPositions[currFrameIndex][-1] <=
                    Line1[0][1] and blob.centerPositions[prevFrameIndex][-2] >= Line2[0][0] and
                    blob.centerPositions[currFrameIndex][-2] >= Line2[0][0] and
                    blob.centerPositions[prevFrameIndex - 1][-1] > Line1[0][1]):
                carCount2 += 1
...
while (True):
    ...
    atLeastOneBlobCrossedTheLine2, carCount2, carCount3 = checkIfBlobs2CrossedTheLine(blobs, Line1, carCount2,
                                                                                      carCount3)
...
while (i < n):
    ...
    while j < len(blobs[list(trackedlist)[i]].centerPositions):
        ...
        file.write("time :" + repr(blobs[list(trackedlist)[i]].timeblob[j]) + " " + "Centrepositions :" + repr(
            blobs[list(trackedlist)[i]].centerPositions[j]) + "\n")
        """ " + "Areablob :" + repr(
            blobs[list(trackedlist)[i]].blobarea[j]) + " " "blobaspectratio :" + repr(
            blobs[list(trackedlist)[i]].aspectrat[j]) + "\n")"""
